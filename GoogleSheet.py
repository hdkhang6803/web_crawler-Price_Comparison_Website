...
def get_used_sheet_name(cate_name, used_sheet):
...
class GoogleSheet:
    def switch_spreadsheet_id(spreadsheet_id):
        if spreadsheet_id == 0:
    ...
    def update_with_data(self, data, used_sheet, cate_name = ""):
      value_range_body = {
        "majorDimension": "DIMENSION_UNSPECIFIED",
        "values": data 
      }

      sheetName = get_used_sheet_name(cate_name, used_sheet)
    ...
    def sort_sheet_by_price(self, sheet_id, cate_name, used_sheet):    
      if 
    ...
    def clear_sheets(self):
      ...
      for sheet in sheet_properties:
          ...
          if sheet_name not in ['Laptop', 'Desktop', 'LinhKien', 'PhuKien']:
              continue
          ...
          try:
              ...
              print(f"Error clearing values in sheet {sheet_name} (id {sheet_id}): {e}")
